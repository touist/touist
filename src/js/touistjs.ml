...
type loc = { line : int; col : int; start_abs : int; end_abs : int }
[@@deriving yojson]
...
type latex_ret = Latex_Succ of string * messages | Latex_Error of messages
[@@deriving yojson]
...
let to_msg (msg : Touist.Err.msg) : msg =
  let open Touist.Err in
  let typ, _, msg, pos = msg in
  let l, c, s, e =
    match pos with None -> (0, 0, 0, 0) | Some l -> get_loc l
  in
    ...
    (object%js
       method linter (txt : Js_of_ocaml.Js.js_string Js.t) : Js.js_string Js.t =
         let ret =
           try
             let _ =
               Touist.Parse.parse_sat (Js.to_string txt)
               |> Touist.Eval.eval ~onlychecktypes:true
             in
             Lint_Succ { messages = [] }
           with Touist.Err.TouistFatal msgs ->
             Lint_Error { messages = [ to_msg msgs ] }
         in
         linter_ret_to_yojson ret
       ...
       method latex (txt : Js.js_string Js.t) : Js.js_string Js.t =
         let ret =
           try
             let ast = Touist.Parse.parse_sat (Js.to_string txt) in
             let latex = Touist.Latex.latex_of_ast ast in
