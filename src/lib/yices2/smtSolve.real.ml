...
let ast_to_yices formula : term * (string, term) Hashtbl.t =
  ...
  let add_term name term =
    if not (Hashtbl.mem vtbl name) then Hashtbl.replace vtbl name term
  in
  ...
  let rec process_terms above_type = function
    ...
    | Not_equal (Int _, Prop x) ->
        add_term x (Term.new_uninterpreted (Type.int ()))
    | Add (Prop x, Float _)
    | Add (Float _, Prop x)
    | Sub (Prop x, Float _)
    | Sub (Float _, Prop x)
    ...
    | Not_equal (Float _, Prop x) ->
        add_term x (Term.new_uninterpreted (Type.real ()))
    | Add (Prop x, Prop y)
    | Sub (Prop x, Prop y)
    | Mul (Prop x, Prop y)
    | Div (Prop x, Prop y)
    ...
    | And (x, y) | Or (x, y) | Xor (x, y) | Implies (x, y) | Equiv (x, y) ->
        process_terms (Type.bool ()) x;
        process_terms (Type.bool ()) y
    | Add (x, Int _)
    | Add (Int _, x)
    | Sub (x, Int _)
    | Sub (Int _, x)
    ...
    | Greater_or_equal (x, y) ->
        process_terms above_type x;
        process_terms above_type y
    | _ -> ()
  in
  let parse = function
    ...
    | And (x, y) ->
        process_terms (Type.bool ()) x;
        process_terms (Type.bool ()) y (*; And (x, y)*)
    | Or (x, y) ->
        process_terms (Type.bool ()) x;
        process_terms (Type.bool ()) y (*; Or  (x, y)*)
    | Xor (x, y) ->
        process_terms (Type.bool ()) x;
        process_terms (Type.bool ()) y (*; Xor (x, y)*)
    | Implies (x, y) ->
        process_terms (Type.bool ()) x;
        process_terms (Type.bool ()) y (*; Implies (x, y)*)
    | Equiv (x, y) ->
        process_terms (Type.bool ()) x;
        process_terms (Type.bool ()) y (*; Equiv (x, y)*)
    | Prop x -> add_term x (Term.new_uninterpreted (Type.bool ()))
    | x ->
        failwith
          ("this cannot be transformed into SMT2: "
          ^ Touist.Pprint.string_of_ast ~debug:true x)
  in
  let rec write (ast : Ast.t) : term =
    match ast with
    | Top -> Term.Bool.true_ ()
    ...
    | Float x ->
        Term.Ratio.parse_float (string_of_float x) (* TODO this is dumb... *)
    ...
    | x ->
        failwith
          ("error smt write: " ^ Touist.Pprint.string_of_ast ~debug:true x)
  in
  parse formula;
  (write formula, vtbl)

let string_of_modelterm model term =
  match term with
  | x when Term.is_bool x ->
      let b = Model.get_bool model x in
      if b then "1" else "0"
...
let string_of_model ?(value_sep = "\n") vtbl model =
  Hashtbl.fold
    (fun name term acc ->
      acc
      ^ (match acc with "" -> "" | _ -> value_sep)
      ^ string_of_modelterm model term
      ^ " " ^ name)
    vtbl ""
...
let solve (logic : string) (formula : Yices2.term) : Yices2.model option =
  ...
  | s ->
      failwith
        ("the status of the yices solver is unexpected: " ^ string_of_status s)

let logic_supported (logic : string) =
  try
    Context.Config.default_for_logic (Context.Config.create ()) logic;
    true
  with
  | YicesError (Error.CTX_UNKNOWN_LOGIC, _) -> false
  | YicesError (Error.CTX_LOGIC_NOT_SUPPORTED, _) -> false
  | YicesError (_code, report) ->
      failwith
        ("[shouldnt happen] instead of returning CTX_UNKNOWN_LOGIC,"
