...
let rec latex_of_ast ?(matrix_instead_of_substack=false) ~full ast =
  ...
  | UnexpProp (x,Some y) -> sprintf "%s_{%s}" (escape_underscore x) (latex_of_commalist "," y)
  | Var (name,ind)   -> sprintf "\\mathbf{%s}%s" (escape_underscore (rm_dollar name))
  ...
  | ArithBinop (x,Div,y) -> sprintf "\\frac{%s}{%s}" (latex_of_ast x) (latex_of_ast y) (* AS: I'd prefer to use \\div, but leaving things as they are now. *)
  | ArithBinop (x,b,y) -> sprintf "%s %s %s" (latex_of_ast x) (arith_binop b) (latex_of_ast y)
  | Not     x     -> "\\neg " ^ latex_of_ast x
  | LogicBinop  (x,b,y) -> sprintf "%s %s %s" (latex_of_ast x) (logic_binop b) (latex_of_ast y)
  | ArithBinrel (x,b,y) -> sprintf "%s %s %s"(latex_of_ast x) (arith_binrel b) (latex_of_ast y)
  ...
  | Bigor (x,y,b,z) ->
      sprintf "\\bigvee\\limits_{%s%s\\in%s%s%s}%s"
        ...
        (if matrix_instead_of_substack then "\\end{matrix}" else "}")
        (latex_of_ast (if z |> Eval.ast_without_layout |> is_binary_op then Layout (Paren, z) else z))
  | Cardinality (c,x,y) -> sprintf "%s(%s,%s)" (cardinality c) (latex_of_ast x) (latex_of_ast y)
  | Let (v,x,c) ->
    let rec unwrap acc ast : string = match ast with
        ...
        | c' ->
            sprintf "\\left(%s\\right)_{\\begin{Bmatrix}%s\\end{Bmatrix}}"
              (latex_of_ast c')
              (acc |> List.fold_left
                        (fun acc (v,x) -> sprintf "%s \\leftarrow %s\\\\ %s" (latex_of_ast v) (latex_of_ast x) acc)
                        "")
    in unwrap [(v,x)] c
  | Affect (v,c) -> (latex_of_ast v) ^ " \\leftarrow " ^ (latex_of_ast c)
  | Layout (l, x) -> layout full (contains_newline x) (latex_of_ast x) l
  | Exists (v,f) -> sprintf "\\exists %s. %s" (latex_of_ast v) (latex_of_ast f)
  | Forall (v,f) -> sprintf "\\forall %s. %s" (latex_of_ast v) (latex_of_ast f)
  | For (var,set,above_f) ->
    let op, prop, f = match Eval.ast_without_layout above_f with
    ...
    in
    sprintf "\\displaystyle\\mathop{%s%s}_{%s\\in %s} .%s" op (latex_of_ast prop) (latex_of_ast var) (latex_of_ast set) (latex_of_ast f)
  | Formula f -> latex_of_ast f
  | SetBuilder (f, vars, sets, cond) -> sprintf "[%s~|~%s\\in %s%s]" (latex_of_ast f) (latex_of_commalist "," vars) (latex_of_commalist " \\times " sets) (match cond with Some c -> ", "^ latex_of_ast c | None -> "")
...
and ast_fun (f:('a -> Ast.t -> 'a)) (acc:'a) ast : 'a =
  ...
  | ArithUnop (_, f)
  | Not f
  | Bigand (_,_,_,f) | Bigor  (_,_,_,f) | Let (_,_,f) | Layout (_, f)
  | Exists (_,f) | Forall (_,f) | For (_,_,f)
      -> acc |> ast_fun' f
  | ArithBinop (_,Mod,_) -> acc
  | ArithBinop (x,_,y)
  | If (_,x,y) | LogicBinop (x,_,y)
  | ArithBinrel (x,_,y)
      -> acc |> ast_fun' x |> ast_fun' y
  | Int _ | Float _ | Bool _ | Top | Bottom | Prop _ | UnexpProp _ | Var _
  | Set _ | Set_decl _ | Card  _ | Cardinality _ | Affect  _
  | Formula _ | SetBuilder _
      -> acc
  (* non-formulas *)
  | SetBinop _ | Range _ | Subset _ | Powerset _
  | In _ | IsEmpty _ -> acc

and contains_newline ast =
  ast |> ast_fun (fun acc ast -> match ast with
    Layout (NewlineAfter, _) | Layout (NewlineBefore, _) -> true | _ -> acc) false
