...
let to_smt2 logic formula =
  ...
  let decl_var name typ = "(declare-fun " ^ name ^ " () " ^ typ ^ ")\n" in
  ...
  let parse = function
    ...
    | Or (x, y) ->
        gen_var "Bool" x;
        gen_var "Bool" y (*; Or  (x, y)*)
    | Xor (x, y) ->
        gen_var "Bool" x;
        gen_var "Bool" y (*; Xor (x, y)*)
    | Implies (x, y) ->
        gen_var "Bool" x;
        gen_var "Bool" y (*; Implies (x, y)*)
    | Equiv (x, y) ->
        gen_var "Bool" x;
        gen_var "Bool" y (*; Equiv (x, y)*)
