...
let to_smt2 logic formula =
  ...
  let rec gen_var typ = function
    ...
    | ArithBinrel (Int _, _, Prop x) -> add_var x "Int"
    | ArithBinop  (Prop x, _, Float _)
    | ArithBinop  (Float _, _, Prop x)
    | ArithBinrel (Prop x, _, Float _)
    | ArithBinrel (Float _, _, Prop x) -> add_var x "Real"
    | ArithBinop  (Prop x, _, Prop y)
    | ArithBinrel (Prop x, _, Prop y) ->
        begin
          try
    ...
    | Not x -> gen_var typ x
    | LogicBinop (x,_,y) -> gen_var "Bool" x; gen_var "Bool" y
    ...
    | ArithBinrel (Float _, _, x) ->
        ...
        if term_expr x then go x else ()
    | ArithBinop  (x, _, y)
    | ArithBinrel (x, _, y) -> gen_var typ x; gen_var typ y
    | _ -> ()
  in

  let parse = function
    | ArithBinrel (x, _, Int _)
    | ArithBinrel (Int _, _, x) -> gen_var "Int" x(*; Equal (x, Int y)*)
    | ArithBinrel (x, _, Float _)
    | ArithBinrel (Float _, _, x) -> gen_var "Real" x(*; Equal (x, Float y)*)
    | LogicBinop  (x,_,y) -> gen_var "Bool" x; gen_var "Bool" y(*; And (x, y)*)
  ...
  let rec write = function
    ...
    | LogicBinop  (x,Equiv,y)    -> write (LogicBinop (LogicBinop (x,Implies,y),And, LogicBinop (y,Implies,x)))
    | LogicBinop  (x,b,y)  -> decl_bin_op (Pprint.logic_binop b) (write x) (write y)
    | ArithUnop (Neg, Int x)   -> "(- " ^ string_of_int (-x) ^ ")"
    | ArithUnop (Neg, Float x) -> "(- " ^ string_of_float (-.x) ^ ")"
    | ArithBinop  (x,b,y)  -> decl_bin_op (Pprint.arith_binop b) (write x) (write y)
    | ArithBinrel (x,Equal,y)     -> decl_bin_op "=" (write x) (write y)
    | ArithBinrel (x,Not_equal,y) -> write (Not (ArithBinrel (x,Equal,y)))
    | ArithBinrel (x,b,y)  -> decl_bin_op (Pprint.arith_binrel b) (write x) (write y)
    | Layout (_, x) -> write x
