...
let ocamlqbf_of_ast (ast : Ast.t) :
    Qbf.QFormula.t * (Qbf.Lit.t, string) Hashtbl.t =
  ...
  let add_lit (name : string) : Qbf.Lit.t =
    ...
    with Not_found ->
      Hashtbl.add str_to_lit name lit;
      Hashtbl.add lit_to_str lit name;
      lit
  in
  let rec of_quant_ast (ast : Ast.t) : QFormula.t =
    match ast with
    | Exists (Prop name, f) ->
        exists [ add_lit name ]
          (if Touist.Qbf.is_unquant f then prop (of_ast f) else of_quant_ast f)
    | Forall (Prop name, f) ->
        forall [ add_lit name ]
          (if Touist.Qbf.is_unquant f then prop (of_ast f) else of_quant_ast f)
  ...
  and of_ast = function
    ...
    | wrong ->
        failwith
          ("[shouldnt happen] when applying 'of_ast', formula still had \
            quantors: "
          ^ Touist.Pprint.string_of_ast ~debug:true wrong)
  in
  let ocamlqbf = of_quant_ast ast in
  (simplify ocamlqbf, lit_to_str)
...
let string_of_assign (assign : assignment) : string =
  match assign with True -> "1" | False -> "0" | Undef -> "?"

(* hidden=false allows to display the tseitlin-added variables. *)
let solve ?(hidden = false)
    ((f, table) : QCNF.t * (Qbf.Lit.t, string) Hashtbl.t) : string option =
  let res = solve ~solver:Quantor.solver f in
  match res with
  ...
  | Sat assign ->
      Some
        (Hashtbl.fold
           (fun lit name acc ->
             if (not hidden) && name.[0] = '&' then acc
             else
               (if acc = "" then "" else acc ^ "\n")
               ^ (assign lit |> string_of_assign)
               ^ " " ^ name)
           table "")
