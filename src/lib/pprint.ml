...
let print_condition pr = function Some c -> " when " ^ pr c | None -> ""

let quantifier_base q v f =
  match q with
  | Exists -> sprintf "exists %s: %s" v f
  | Forall -> sprintf "forall %s: %s" v f

let quantifier_utf8 q v f =
  match q with
  | Exists -> sprintf "∃%s.%s" v f
  | Forall -> sprintf "∀%s.%s" v f

let quantifier ?(utf8 = false) =
  if utf8 then quantifier_utf8 else quantifier_base
...
let rec string_of_ast ?(utf8 = false) ?(show_var = fun _ -> "") ?(debug = false)
  ...
  | ArithBinop (x, b, y) ->
      sprintf "(%s %s %s)" (of_ast x) (arith_binop b) (of_ast y)
  | Not x when utf8 -> "¬" ^ of_ast x
  | Not x -> "not " ^ of_ast x
  | LogicBinop (x, b, y) ->
      sprintf "(%s %s %s)" (of_ast x) (logic_binop ~utf8 b) (of_ast y)
  | ArithBinrel (x, b, y) ->
      sprintf "%s %s %s" (of_ast x) (arith_binrel ~utf8 b) (of_ast y)
  | SetBinop (x, b, y) when utf8 ->
      sprintf "%s %s %s" (of_ast x) (set_binop_utf8 b) (of_ast y)
  | SetBinop (x, b, y) ->
      sprintf "%s(%s, %s)" (set_binop_base b) (of_ast x) (of_ast y)
  ...
  | Bigand (x, y, cond, z) ->
      sprintf "bigand %s in %s%s:\n%s\nend\n" (of_ast_list "," x)
        (of_ast_list "," y)
        (print_condition of_ast cond)
        (of_ast z)
  | Bigor (x, y, cond, z) ->
      sprintf "bigor %s in %s%s:\n%s\nend\n" (of_ast_list "," x)
        (of_ast_list "," y)
        (print_condition of_ast cond)
        (of_ast z)
  | Cardinality (c, x, y) ->
      sprintf "%s(%s,%s)" (cardinality c) (of_ast x) (of_ast y)
  ...
  | Quantifier (q, v, f) -> quantifier ~utf8 q (of_ast v) (of_ast f)
  | For (v, c, f) -> sprintf "for %s in %s:%s" (of_ast v) (of_ast c) (of_ast f)
  | Formula f -> "\"" ^ of_ast f ^ "\""
  | SetBuilder (f, vars, sets, cond) ->
      sprintf "[%s for %s in %s%s]" (of_ast f) (of_ast_list "," vars)
        (of_ast_list "," sets)
        (print_condition of_ast cond)
...
and string_of_ast_type ?(debug = false) (ast : Ast.t) : string =
  ...
  | Quantifier (Exists, _, _) -> "exists"
  | Quantifier (Forall, _, _) -> "forall"
  | For (_, _, _) -> "for"
