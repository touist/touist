...
let lexer buffer : Lexing.lexbuf -> Parser.token =
  let tokens = ref [] in
  (* tokens stored to be processed (see above) *)
  fun lexbuf ->
    match !tokens with
    | x :: xs ->
        tokens := xs;
        x (* tokens isn't empty, use one of its tokens *)
    | [] -> (
        (* tokens is empty, we can read a new token *)
        let t = Lexer.token lexbuf in
        buffer :=
          ParserReport.update !buffer (lexbuf.lex_start_p, lexbuf.lex_curr_p);
        match t with
...
*)
let parse parser ?(debug = false) filename (text : string) : Ast.t =
  let buffer = ref ParserReport.Zero in
  let lexbuf = Lexing.from_string text in
  lexbuf.lex_curr_p <-
    { lexbuf.lex_curr_p with pos_fname = filename; pos_lnum = 1 };
  let checkpoint = parser lexbuf.lex_curr_p
  and supplier =
    Parser.MenhirInterpreter.lexer_lexbuf_to_supplier (lexer buffer) lexbuf
  and succeed ast = ast
  and fail checkpoint =
    let msg = ParserReport.report text !buffer checkpoint debug
    and loc =
      ParserReport.area_pos !buffer
      (* area_pos returns (start_pos,end_pos) *)
    in
    fatal (Error, Parse, msg, Some loc)
  in
  let ast =
...
let parse_sat ?debug_syntax:(d = false) ?(filename = "foo.touistl") text =
  parse Parser.Incremental.touist_simple ~debug:d filename text

(** Same for [Parser.Incremental.touist_simple] *)
let parse_smt ?debug_syntax:(d = false) ?(filename = "foo.touistl") text =
  parse Parser.Incremental.touist_smt ~debug:d filename text
...
let string_of_file (name : string) : string = string_of_chan (open_in name)
