...
let string_of_type = function Warning -> "warning" | Error -> "error"
...
let get_loc loc : int * int * int * int =
  let s, e = loc in
  (s.pos_lnum, s.pos_cnum - s.pos_bol + 1, s.pos_cnum, e.pos_cnum)
...
let replace (placeholder : char -> string) text =
  ...
  let rec replace cur_pos =
    try
      let next_pos =
        Re.Str.search_forward (Re.Str.regexp "%[a-zA-Z]") text cur_pos
      in
      (*Printf.printf "cur=%d next=%d len=%d (%s)\n" cur_pos next_pos (String.length text) text;*)
      String.sub text cur_pos (next_pos - cur_pos)
      ^ (text.[next_pos + 1] |> placeholder)
      ^
      if next_pos + 2 <= String.length text - 1 then replace (next_pos + 2)
      else ""
    with Not_found -> String.sub text cur_pos (String.length text - cur_pos)
  in
  replace
    (if placeholder 'l' = "" then
     Re.Str.search_forward (Re.Str.regexp "%[^flcLCbB]") text 0
    else 0)
...
let format_width _ width text =
  let rec format prev_indent from_pos =
    let cur_indent =
      try Re.Str.search_forward (Re.Str.regexp "[^ ]") text from_pos - from_pos
      with Not_found -> 0
    in
    let wrap_pos =
      let newline_pos =
      ...
      in
      if newline_pos > from_pos + width then from_pos + width else newline_pos
    in
    ...
    | _ when text.[wrap_pos] = '\n' ->
        (* wrap at a \n symbol *)
        spaces prev_indent
        ^ String.sub text from_pos (wrap_pos - from_pos)
        ^ "\n"
        ^ format 0 (wrap_pos + 1)
    | _ ->
        (* wrap at any point because width is too large *)
        let last_space =
          try
            Re.Str.search_backward
              (Re.Str.regexp "\\( \\|: \\|, \\|. \\)")
              text wrap_pos
          with Not_found -> wrap_pos
        in
        let last_space_end =
          last_space + if text.[last_space] = ' ' then 0 else 1
        in
        spaces prev_indent
        ^ String.sub text from_pos (last_space_end - from_pos)
        ^ "\n"
        ^ format (cur_indent + prev_indent) (last_space_end + 1)
  in
  if width != 0 then format 0 0 else text

let string_of_msg ?(width = !wrap_width) ?(color = !color) ?(fmt = !format)
    (message : msg) =
  let color_backquote text =
    let colorize str = "\x1b[33m" ^ str ^ "\x1b[0m" in
    Re.Str.global_substitute
      (Re.Str.regexp "`\\([^`]+\\)`")
      (fun s -> "`" ^ colorize (Re.Str.matched_group 1 s) ^ "`")
      text
  in
  let color_quoted text =
    let colorize str = "\x1b[33m" ^ str ^ "\x1b[0m" in
    Re.Str.global_substitute
      (Re.Str.regexp "'\\([^']*\\)'")
      (fun s ->
        let s = Re.Str.matched_group 1 s in
        if String.length s = 0 then "''" else "'" ^ colorize s ^ "'")
      text
  in
  let color_code text =
    let colorize str = "\x1b[37m" ^ str ^ "\x1b[0m" in
    Re.Str.global_substitute
      (Re.Str.regexp "^\\(    +.*\\)$")
      (fun s -> colorize (Re.Str.matched_group 1 s))
      text
  ...
  let color_type text =
    let colorize str =
      ...
      | str -> str
    in
    Re.Str.substitute_first
      (Re.Str.regexp "\\(error\\|warning\\)")
      (fun s -> colorize (Re.Str.matched_group 1 s))
      text
  in
  let color_all text =
    if color then
      text |> color_code |> color_backquote |> color_quoted |> color_type
    else text
  in
  let typ, _, text, loc = message in
  replace (all_placeholders loc typ color text) fmt
  |> format_width color width |> color_all

let warn msg =
  if !discard_warnings then ()
  else Printf.fprintf stderr "%s" (string_of_msg msg)

let fatal msg = raise @@ TouistFatal msg

let _ =
  Printexc.register_printer (fun ex ->
      match ex with TouistFatal msg -> Some (string_of_msg msg) | _ -> None)
