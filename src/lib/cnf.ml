...
let rec is_clause (ast : Ast.t) : bool =
  ...
  | LogicBinop (x, Or, y) -> is_clause x && is_clause y
  | LogicBinop (_, And, _) -> false
  | _ -> false
...
let rec push_lit (lit : Ast.t) (cnf : Ast.t) : Ast.t =
  ...
  | Top -> Top
  | Bottom -> lit
  | Prop x -> LogicBinop (lit, Or, Prop x) (* p,i = prefix, indices *)
  | Not (Prop x) -> LogicBinop (lit, Or, Not (Prop x))
  | LogicBinop (x, And, y) -> LogicBinop (push_lit lit x, And, push_lit lit y)
  | LogicBinop (x, Or, y) -> LogicBinop (lit, Or, LogicBinop (x, Or, y))
  | ast ->
      failwith
        ("[shouldnt happen] this doesn't seem to be a formula: '"
...
let fresh_dummy () =
  ...
  Prop ("&" ^ string_of_int !dummy_term_count)
...
let print_debug (prefix : string) depth (formulas : Ast.t list) : unit =
  ...
  in
  print_endline
    (indent depth ^ string_of_int depth ^ " " ^ prefix ^ string_of_asts formulas)
...
and to_cnf depth (stop : stop) (ast : Ast.t) : Ast.t =
  ...
  if match stop with Yes 0 -> true | _ -> false then ast
  else
    (* See (1) above*)
    let to_cnf_once =
      to_cnf (depth + 1) (match stop with Yes i -> Yes (i - 1) | No -> Yes 1)
    in
    let to_cnf =
      to_cnf (depth + 1) (match stop with Yes i -> Yes (i - 1) | No -> No)
    in
    let cnf =
      match ast with
      | Top when depth = 0 ->
          let t = fresh_dummy () in
          LogicBinop (t, Or, Not t)
          (* See (2) above *)
      | Top -> Top
      | Bottom when depth = 0 ->
          let t = fresh_dummy () in
          LogicBinop (t, And, Not t)
          (* See (2) *)
      | Bottom -> Bottom
      | Prop x -> Prop x
      | LogicBinop (x, And, y) -> (
      ...
      | Not x -> (
          ...
          | Not x -> to_cnf x
          | LogicBinop (x, And, y) ->
              to_cnf (LogicBinop (Not x, Or, Not y)) (* De Morgan *)
          | LogicBinop (x, Or, y) ->
              LogicBinop (to_cnf (Not x), And, to_cnf (Not y)) (* De Morgan *)
          | _ -> to_cnf (Not (to_cnf_once x)) (* See (1) above*))
      | LogicBinop (x, Or, y) -> (
          if !debug then print_debug "Or: " depth [ x; y ];
          let x, y = (to_cnf x, to_cnf y) in
          match (x, y) with
          | Bottom, z | z, Bottom -> z
          | Top, _ | _, Top -> Top
          ...
          | x, y when is_clause x && is_clause y -> LogicBinop (x, Or, y)
          | x, y ->
              (* At this point, either x or y is a conjunction
                 => Tseytin transform (see explanations below) *)
              let new1, new2 = (fresh_dummy (), fresh_dummy ()) in
              LogicBinop
                ( LogicBinop (new1, Or, new2),
                  And,
                  LogicBinop (push_lit (Not new1) x, And, push_lit (Not new2) y)
                )
          ...
          *))
      | LogicBinop (x, Implies, y) -> to_cnf (LogicBinop (Not x, Or, y))
      ...
      | LogicBinop (x, Xor, y) ->
          to_cnf
            (LogicBinop
               (LogicBinop (x, Or, y), And, LogicBinop (Not x, Or, Not y)))
      | _ ->
          failwith
            ("[shouldnt happen] this doesn't seem to be a formula: '"
            ^ string_of_ast ~debug:true ast
...
let clauses_of_cnf (neg : 'a -> 'a) (fresh : unit -> 'a) (ast : Ast.t) :
  ...
  let lit_to_str = Hashtbl.create 500 in
  (* this duplicate is for the return value *)
  let rec process_cnf ast : 'a list list =
    match ast with
    | LogicBinop (x, And, y) -> process_cnf x @ process_cnf y
    | x when is_clause x -> [ process_clause x ]
    | _ ->
        failwith
          ("CNF: was expecting a conjunction of clauses but got '"
          ^ string_of_ast ~debug:true ast
          ^ "'")
  and process_clause (ast : Ast.t) : 'a list =
    match ast with
    | Prop str -> [ gen_lit str ]
    | Not (Prop str) -> [ neg (gen_lit str) ]
    | LogicBinop (x, Or, y) -> process_clause x @ process_clause y
    | _ ->
        failwith
          ("CNF: was expecting a clause but got '"
          ^ string_of_ast ~debug:true ast
          ^ "'")
  and gen_lit (s : string) : 'a =
    ...
    with Not_found ->
      ...
      lit
  in
  let clauses = process_cnf ast in
  (clauses, lit_to_str, str_to_lit)

let print_table (int_of_lit : 'a -> int) (out : out_channel) ?(prefix = "")
    (table : ('a, string) Hashtbl.t) =
  let print_lit_and_name lit name =
    Printf.fprintf out "%s%s %d\n" prefix name (int_of_lit lit)
  in
  Hashtbl.iter print_lit_and_name table

let print_clauses (out : out_channel) ?(prefix = "") (str_of_lit : 'a -> string)
    (clauses : 'a list list) : unit =
  let rec string_of_clause (cl : 'a list) =
    match cl with
    | [] -> "0"
    | cur :: next -> str_of_lit cur ^ " " ^ string_of_clause next
  and print_listclause (cl : 'a list list) =
    match cl with
    | [] -> ()
    | cur :: next ->
        Printf.fprintf out "%s%s\n" prefix (string_of_clause cur);
        print_listclause next
  in
