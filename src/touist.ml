...
let solve_ext common_opt show_hidden output lit_tbl lit_abs lit_sign lit_of_int (print_dimacs:out_channel -> unit) cmd =
  ...
  | Unix.WEXITED 10 when List.length valuated_lits > 0 ->
    (lit_tbl |> Hashtbl.iter (fun lit lit_name ->
     ...
     exit_with OK)
  | Unix.WEXITED 127 -> (Printf.eprintf
                           "Command '%s' not found (try with --debug)\n" cmd;
                         exit_with SOLVER_UNKNOWN)
  ...
  | Unix.WEXITED c -> Printf.eprintf
                        ...
                        cmd c; exit_with SOLVER_UNKNOWN;
...
let main (input,input_f) (output,output_f : string*out_channel) (lang,mode) common_opt =
  ...
  TouistErr.wrap_width := common_opt.wrap_width;
  TouistErr.format := common_opt.error_format;
  ...
  try
    ...
    if (match lang with Smt l -> TouistSmtSolve.enabled && not (TouistSmtSolve.logic_supported l) | _->false)
    ...
    let ast_plain text =
      match lang with
      | Sat -> TouistParse.parse_sat ~debug_syntax ~filename:input text
      | Smt _ -> TouistParse.parse_smt ~debug_syntax ~filename:input text
      | Qbf -> TouistParse.parse_qbf ~debug_syntax ~filename:input text
    in begin
      ...
      | _, Latex {mode;linter} -> let ast_plain = ast_plain input_text in
        (if linter then ast_plain |> TouistEval.eval ~smt ~onlychecktypes:true |>ignore);
      ...
      | _, Translate {linter=true} ->
        (ast_plain input_text |> TouistEval.eval ~smt ~onlychecktypes:true |> ignore; exit_with OK)
      | _,Show -> let ast = match lang with
          | Sat   -> TouistParse.parse_sat ~debug_syntax ~filename:input input_text |> TouistEval.eval ~smt
          | Smt _ -> TouistParse.parse_smt ~debug_syntax ~filename:input input_text |> TouistEval.eval ~smt
          | Qbf   -> TouistParse.parse_qbf ~debug_syntax ~filename:input input_text |> TouistEval.eval ~smt
        in (Printf.fprintf output_f "%s\n" (TouistPprint.string_of_ast ~utf8:true ast); exit_with OK)
      (* A. solve has been asked *)
      | Sat, Solve {equiv=Some (input2,input2_f)} ->
        let solve inputchan =
          let ast = TouistParse.parse_sat ~debug_syntax ~filename:input (string_of_chan inputchan) |> TouistEval.eval in
          let models = TouistCnf.ast_to_cnf ~debug_cnf ast |> TouistSatSolve.minisat_clauses_of_cnf |> TouistSatSolve.solve_clauses ~verbose:common_opt.debug
          in models
        in
        let models = solve input_f and models2 = solve input2_f in
        (match TouistSatSolve.ModelSet.equal !models !models2 with
      ...
      | Sat, Solve {equiv=None; limit; count; interactive} ->
        let ast = TouistParse.parse_sat ~debug_syntax ~filename:input input_text |> TouistEval.eval in
        let clauses,table = TouistCnf.ast_to_cnf ~debug_cnf ast |> TouistSatSolve.minisat_clauses_of_cnf
        in
        let models =
          begin
            if count then TouistSatSolve.solve_clauses ~verbose:common_opt.debug (clauses,table)
            else
              ...
              in
              TouistSatSolve.solve_clauses ~verbose:common_opt.debug ~print:print_model
      ...
      | Sat, Translate {linter=false; table=Some (table,table_f)} -> (* B. solve not asked: print the Sat file *)
        let ast = TouistParse.parse_sat ~debug_syntax ~filename:input input_text |> TouistEval.eval in
        let clauses,tbl = TouistCnf.ast_to_cnf ~debug_cnf ast |> TouistSatSolve.minisat_clauses_of_cnf
        in TouistSatSolve.print_dimacs ~debug_dimacs (clauses,tbl)
      ...
      | Sat, SolveExt {cmd; hidden} ->
        ...
        in cmd |> solve_ext common_opt hidden output_f tbl (Minisat.Lit.abs) (Minisat.Lit.sign)
             ...
             (TouistSatSolve.print_dimacs ~debug_dimacs (clauses,tbl))
      | Smt logic, Translate _ ->
        let ast = TouistParse.parse_smt ~debug_syntax ~filename:input input_text
                  |> TouistEval.eval ~smt in
        ast |> TouistSmt.to_smt2 logic  |> Buffer.output_buffer output_f;
        exit_with OK
      | Smt logic, Solve _ ->
        ...
        (match TouistSmtSolve.solve logic yices_form with
      ...
      | Qbf, mode -> begin
          let ast = TouistParse.parse_qbf ~debug_syntax ~filename:input input_text
                    |> TouistEval.eval ~smt in
          ...
          begin match mode with
            ...
            | Translate {table=Some (table,table_f)} -> (* --qbf: we print QDIMACS *)
              ...
              (TouistQbf.print_qdimacs ~debug_dimacs
            ...
            | Solve {hidden} -> (* --qbf + --solve: we solve using Quantor *)
              ...
              (match TouistQbfSolve.solve ~hidden (qcnf,table) with
...
let latex_flag =
  Arg.(value & opt (some (enum [("mathjax",Mathjax);("document",Document)])) None
    ...
    for $(i,MODE) are `mathjax' and `document'.|})
